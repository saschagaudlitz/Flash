"""
https://github.com/hihihihiwsf/AST/
"""
...
class AST(nn.Module):
    ...
    def __init__(self, d_model, n_classes, n_cov_dim, n_embedding_dim, n_layers, attention_type, d_ff, predict_steps, train_window, dropout, h):
        super().__init__()
        c = copy.deepcopy

        position = PositionalEncoding(d_model=d_model, dropout=dropout)
        generator = Generator(d_model=d_model)
        embedding = Embedding(position=position, d_model=d_model, n_cov_dim=n_cov_dim, n_classes=n_classes, n_embedding_dim=n_embedding_dim)
        attention = MultiHeadedAttention(d_model=d_model, h=h, attention_type=attention_type)
        feedforward = PositionwiseFeedForward(d_model=d_model, d_ff=d_ff, dropout=dropout)

        encoder_layer = EncoderLayer(c(attention), c(feedforward), dropout=dropout, d_model=d_model)
        self.encoder = Encoder(encoder_layer, n_layers=n_layers)

        decoder_layer = DecoderLayer(c(attention), c(attention), c(feedforward), dropout=dropout, d_model=d_model)
        self.decoder = Decoder(layer=decoder_layer, n_layers=n_layers)

        self.model = EncoderDecoder(embedding=embedding, encoder=self.encoder, decoder=self.decoder, generator=generator, predict_steps=predict_steps)
        self.discriminator = Discriminator(train_window=train_window)
    ...
    def loss(self, *args, **kwargs):
        ...
        generator_loss = loss_quantile(q50, labels, torch.tensor(0.5)) + 0.1 * self.adversarial_loss(self.discriminator(fake_input), valid)
        discriminator_real_loss = self.adversarial_loss(self.discriminator(labels_batch), valid)
        discriminator_fake_loss = self.adversarial_loss(self.discriminator(fake_input), fake)
...
class EncoderDecoder(nn.Module):
    def __init__(self, predict_steps, embedding, encoder, decoder, generator):
    ...
    def encode(self, x, idx):
        ...
        embedded = self.embedding(x, idx)
        encoder_out = self.encoder(embedded, None)
    ...
    def decode(self, memory, x, idx, src_mask):
        ...
        embedded = self.embedding(x, idx)
        decoder_out = self.decoder(embedded, memory, None, tgt_mask)
        return decoder_out
...
class Embedding(nn.Module):
    def __init__(self, position, d_model, n_cov_dim, n_classes, n_embedding_dim):
        super(Embedding, self).__init__()
        self.position = position
        self.d_model = d_model
        self.n_cov_dim = n_cov_dim
        self.n_classes = n_classes
        self.n_embedding_dim = n_embedding_dim

        self.embedding = nn.Embedding(n_classes, n_embedding_dim)
        ...
        '''
        self.embed1 = nn.Linear(n_embedding_dim + n_cov_dim + 1, d_model)
...
class Generator(nn.Module):
    def __init__(self, d_model):
        ...
        self.q50 = nn.Linear(d_model, 1)
        self.q90 = nn.Linear(d_model, 1)
...
class EncoderLayer(nn.Module):
    def __init__(self, self_attn, feed_forward, dropout, d_model):
...
class Decoder(nn.Module):
    ...
    def __init__(self, n_layers, layer):
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, self_attn, src_attn, feed_forward, dropout, d_model):
...
def attention(query, key, value, attention_type, mask=None, dropout=None, alpha=None):
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, d_model, h, attention_type, dropout=0.2):  # TODO : h , dropout
        ...
        self.scores = None
        self.alpha_choser = AlphaChooser(h)
        self.alpha = None
        self.attention_type = attention_type
    ...
    def forward(self, query, key, value, mask=None):
        ...
        if self.attention_type == 'entmax':
            self.alpha = self.alpha_choser()
        x, self.scores, self.attn = attention(query, key, value, self.attention_type, mask=mask, dropout=self.dropout, alpha=self.alpha)
...
def loss_quantile(mu, labels, quantile):
    loss = 0
