...
def main(args):
    ...
    model = build_model(n_dim, args.n_layers, args.n_hidden_features)
    optimizer = optim.Adam(model.parameters(), lr=args.lr)

    # we do it once outside the loop for the validation set, because this operation takes a couple of seconds
    ...
    with mlflow.start_run() as run:
        ...
        for epoch_idx in pbar:
            for batch_idx, batch in enumerate(train_loader):
                optimizer.zero_grad()
                loss = -model.log_prob(inputs=batch[0]).mean()
                loss.backward()
                optimizer.step()

            X_pred = model.sample(len(X_val)).detach().numpy()
            ...
            if kendall < best_kendall:
                ...
                mlflow.pytorch.log_model(model, 'best_kendall')
            if ad_mean < best_ad_mean:
                best_ad_mean = ad_mean
                mlflow.pytorch.log_model(model, 'best_ad_mean')
        ...
        X_test_pred = model.sample(len(X_test)).detach().numpy()
